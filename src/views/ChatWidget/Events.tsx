...
const EventList: React.FC = () => {
    ...
    return (
        <>
            {!selectedEvent ? <div className="px-4 overflow-auto">
                <div className="flex items-center rounded p-1">
                    ...
                    <div className="flex flex-col px-2">
                        <p className="text-xl font-semibold">Events near You :</p>
                    </div>

                </div>
                <div className="grid grid-cols-2 gap-4">
                    {events.map((event) => (
                        <div key={event.id} onClick={() => setSelectedEvent(event)} className="cursor-pointer bg-white rounded-lg shadow-md overflow-hidden">
                            <img
                                src={event.imageUrl}
                                alt={event.title}
                            ...
                            />
                            <div className="p-4">
                                <p className="text-sm text-gray-500">{moment(event.date).format('MMM Do')}, {event.time}</p>
            ...
            </div> : <EventDetails event={selectedEvent} setSelectedItem={setSelectedEvent} />
            }
        </>
    );
};
