...
function Home() {
   ...
   const [messages, setMesages] = useState<string[]>(['Hi, Brandon. Welcome Back'])
   const [smaResult, setSmsResult] = useState<string>(`Hey John, new strain Gelato Zkittlez at Green Rose, 20% off coupon. It's like Pink Runtz you loved - phenotype of Runtz crossed with Gelato & Zkittlez. Don't miss this deal!`)
   ...
   function playHandler(){
      if(!prompts) return
      setLoading(true)
      axios.post('https://cannabis-marketing-chatbot-224bde0578da.herokuapp.com/chat', {message: prompts})
      .then((res)=>{
         ...
         <div className=" bg-[#1E1E1E] min-h-screen lg:w-[80%] overflow-hidden">
            <div className="xl:h-[75%] lg:grid [@media(min-width:1020px)]:grid-cols-2 flex-grow gap-3 py-9 [@media(min-width:600px)]:pr-10 pr-4 pl-3">
               <div className="h-[100%] mt-0 md:mt-7">
                  ...
                  <div className="min-h-[400px] h-full [@media(min-width:1281px)]:min-h-[590px] bg-[#1C1919] border border-white rounded-lg px-5 py-10">
                     <p className="mb-10 flex flex-wrap gap-2">
                        <button className="bg-[#636363] px-3 py-2 text-white font-istok-web font-medium text-sm  rounded-3xl text-[14px]">Search Customer(s)</button>
                        <button className="bg-[#07e81a] px-3 py-2 text-white font-istok-web font-medium text-sm rounded-3xl text-[14px]">Run SMS</button>
                        <button className="bg-[#659422] px-3 py-2 text-white font-istok-web font-medium text-sm rounded-3xl text-[14px]">Run Email</button>
                        <button className="bg-[#304e05] px-3 py-2 text-white font-istok-web font-medium text-sm rounded-3xl text-[14px]">Create Content</button>
               ...
               <div className="h-[100%] mt-5 md:m-0 " >
                  <CannabotWorkspace smsResults={smaResult} streamOutput={streamOutput} />
               </div>
            ...
            <div className="flex flex-col items-center justify-center my-5">
               <div className="flex mb-5 [@media(min-width:600px)]:flex-row flex-col items-center justify-center gap-4">
                  <div className="md:flex gap-4">
                     <button onClick={playHandler} className="bg-emerald-500 lg:my-0 my-3 flex flex-col items-center justify-center border-none py-1.5 w-[100px] md:px-10 rounded-md border border-[#999999]">
